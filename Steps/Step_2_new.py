...
class Simulator:
    ...
    def rewards_per_userclass(self, user_class):
        ...
        for p in range(self.n_products):
            for i in range(self.n_products):
                if user_class.graph_proba[p][i] != 0:
                    total_graph_weights[p] *= user_class.graph_proba[p][i]
                prod_reward[p] += cf.margin[p] * user_class.conversion_rates[p] * user_class.user_num[p + 1] * total_graph_weights[p] * user_class.get_sold_items()
    ...
    def Greedy_alg(self, users):
        ...
        price_hist = []
        reward_hist = []
        max_reward = np.sum(self.rewards_per_userclass(self, users))
        ...
        while max_reward  > prev and np.sum(bool_max_prices) < 5:
            ...
            for i in range(self.n_products):
                if price_index[i] < 3:
                    price_index[i]+=1
                    sum_rewards[i] = np.sum(self.rewards_per_userclass(self, users))
                    price_index[i] -=1
                else:
