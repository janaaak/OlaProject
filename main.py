...
#from Steps.Step_2_new import step_2
...
def main():
    ...
    while True:
        ...
        if step == 2:
            results = sim.Greedy_alg(sim.user_classes[0])
            print(results[0])
            print(results[1])
            print(results[2])
            opt, _, best_price_conf = sim.bruteforce()
            opt_greedy = results[1][len(results[1])-1]
            max_price_conf = results[0][len(results[0])-1]
            #opt_per_product, max_price_conf = step_2()
            print("\nRevenue provided by the greedy algorithm:", np.sum(opt_greedy),
        ...
        #     break
            ...
            plot_regret(opt_final, rewardsSW, rewardsCD,540, step=6)
            plot_reward(opt_final, rewardsSW, rewardsCD, 540, step=6)
            break
...
def plot_regret(opt, rewardsUCB_exp, time_horizon, bound=0, step=0):
    plt.figure(0)
    if step == 6:
...
def plot_reward(opt, rewardsUCB_exp, time_horizon, step=0):
    plt.figure(0)
    plt.xlabel("t")
    plt.ylabel("Reward")
