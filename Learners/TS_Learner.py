...
class TS_Learner(Learner):
    ...
    def __init__(self, n_arms, alpha=None, items=None, graph=None):
        ...
        self.total_views = np.zeros(n_arms)
    ...
    def update(self, pulled_arm, reward, buyers, views, alpha=None, items=None, graph=None):
        self.t += 1
        self.total_views[pulled_arm] += views
        self.update_observations(pulled_arm, reward)

        self.beta_parameters[pulled_arm, 0] = self.beta_parameters[pulled_arm, 0] + buyers.astype(int)
        self.beta_parameters[pulled_arm, 1] = self.beta_parameters[pulled_arm, 1] + views.astype(int) - buyers.astype(
            int)
        ...
        if np.all(graph != None):
            ...
            self.graph[pulled_arm] = (self.graph[pulled_arm] * (self.arm_counter[pulled_arm] - 1) + graph) / \
                                     self.arm_counter[pulled_arm]
